...
if [[ -d "$LIB_SRC" ]] && [[ -n "$(ls -A "$LIB_SRC" 2>/dev/null)" ]]; then
  echo "Installing bundled libraries to $LIB_DST"
  sudo mkdir -p "$LIB_DST"
  sudo cp -P "$LIB_SRC"/* "$LIB_DST/"
  sudo chmod 0755 "$LIB_DST"
  sudo ldconfig
  echo "✓ Bundled libraries installed"
else
  echo "No bundled libraries found — runtime dependencies must be installed separately"
fi
