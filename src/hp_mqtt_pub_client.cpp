...
HpMqttPub::HpMqttPub(const std::string& brokerAddress, const std::string& clientId)
    : brokerAddress_(brokerAddress),
      clientId_(clientId),
      site_("site"),  // default site name
      deviceId_(""),  // must be set by user
      hasCredentials_(false),
      hasWill_(false) {
    
    try {
        client_ = std::make_unique<mqtt::async_client>(brokerAddress_, clientId_);
        std::cout << "MQTT client created for broker: " << brokerAddress_ << std::endl;
...
std::string HpMqttPub::buildBaseTopic() const {
    if (deviceId_.empty()) {
        throw std::runtime_error("Device ID not set. Call setDeviceId() before publishing.");
    }
...
void HpMqttPub::setCredentials(const std::string& username, const std::string& password) {
    username_ = username;
    password_ = password;
